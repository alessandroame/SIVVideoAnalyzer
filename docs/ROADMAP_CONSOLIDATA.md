...
3. **Fine Giornata / Fine Corso (Export per i Piloti)**:
   - Produzione su chiavetta dei video per i piloti:
     - Singoli video per ciascun volo (con capitoli e sottotitoli).
...
```mermaid
flowchart TD
    A[Fase 1: Modello Dati & Manifest Portatile] --> B[Fase 2: Raggruppamento per Volo & Nuova UI Step 1/2]
    B --> C[Fase 3: Player Debriefing Immediato Pilota + Volo]
...
### 🟢 FASE 2: Raggruppamento per Volo & Nuova UI Piloti
> **Obiettivo**: Consentire l'inserimento dettagliato dei piloti e raggruppare automaticamente i video in sessioni di volo.
...
- [ ] **2.2 Algoritmo Raggruppamento per Volo (`core/flight_grouper.py`)**:
  - Ordinamento cronologico delle clip di ciascun pilota.
  - Rilevamento automatico cambio volo basato su time-gap (> 20-30 min) tra le riprese.
- [ ] **2.3 Revisione per Volo nello Step 2**:
  - Aggiunta della colonna *Numero di Volo* (`Volo 1`, `Volo 2`, ...) nella tabella a schermo intero.
  - Possibilità per l'istruttore di spostare un video da un volo all'altro con 1 click.
  - **Sostituzione del tasto finale**: *"Conferma ed Entra nel Debriefing ➡"* (salto istantaneo a Step 3 senza attendere rendering).

---

### 🟢 FASE 3: Player Debriefing Immediato (Pilota + Volo)
> **Obiettivo**: Permettere all'istruttore di mostrare subito il volo appena concluso a lezione.
...
- [ ] **3.1 Doppio Selettore nello Step 3**:
  ...
  - Pulsanti / Selettore **Volo**: `[ ✈ Volo 1 | ✈ Volo 2 | ✈ Volo 3 ]`.
- [ ] **3.2 Riproduzione Diretta delle Clip del Volo**:
  - Caricamento istantaneo delle clip native del volo selezionato nel player QMediaPlayer.
  - Tabella capitoli sincronizzata: doppio click su una manovra salta direttamente al minutaggio esatto della clip corrispondente.
...
- [ ] **5.2 Opzioni di Formato**:
  - *Opzione A*: Un video MP4 per ogni volo con capitoli YouTube (`.txt`).
